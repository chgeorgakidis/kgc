    ...
    while True:
        ...
        with open(filename, 'a') as data_file:
            ...
            if no_unbound == 1:
                ...
                if unbound_pos == 0:
                    query_cypher = 'MATCH (x1)-[:' + rel1 + ']->(x2)-[:' + rel2 + ']->(x3), (x1)-[:P1417]->(n1), (x3)-[:P1417]->(n3) WHERE (apoc.text.levenshteinSimilarity(n3.value, \"' + str(el_split[5]) + '\")) >= ' + str(sim2) + ' RETURN COUNT(*)'
                else:
                    query_cypher = 'MATCH (x1)-[:' + rel1 + ']->(x2)-[:' + rel2 + ']->(x3), (x1)-[:P1417]->(n1), (x3)-[:P1417]->(n3) WHERE (apoc.text.levenshteinSimilarity(n1.value, \"' + str(el_split[2]) + '\")) >= ' + str(sim1) + ' RETURN COUNT(*)'
            else:
                query_cypher = 'MATCH (x1)-[:' + rel1 + ']->(x2)-[:' + rel2 + ']->(x3), (x1)-[:P1417]->(n1), (x3)-[:P1417]->(n3) WHERE (apoc.text.levenshteinSimilarity(n1.value, \"' + str(el_split[2]) + '\")) >= ' + str(sim1) +  ' AND (apoc.text.levenshteinSimilarity(n3.value, \"' + str(el_split[5]) + '\")) >= ' + str(sim2) + ' RETURN COUNT(*)'
            ...
            if no_unbound == 0:
                query = x1_id + '-' + rel1 + '-' + str(sim1) + '-*-' + rel2 + '-' + x2_id + '-' + str(sim2) + ',' + str(cardinality)
            elif no_unbound == 1:
                if unbound_pos == 0:
                    query = '*' + '-' + rel1 + '-1.0' + '-*-' + rel2 + '-' + x2_id + '-' + str(sim2) + ',' + str(cardinality)
                elif unbound_pos == 1:
                    query = x1_id + '-' + rel1 + '-' + str(sim1) + '-*-' +rel2 + '-*' + '-' + str(sim2) + ',' + str(cardinality)
...
if __name__ == '__main__':
    ...
    filename = 'training_wiki_chain_super_sim.csv'

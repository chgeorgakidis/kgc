...
def run_process(i):
    ...
    while True:
        ...
        with open(filename, 'a') as data_file:
            ...
            query_cypher = 'MATCH '
            for i in range(joins):
                query_cypher += '(x1:Entity)-[:' + rels[i] + ']->(x' + str(i+2) + ')-[:P1417]->(n' + str(i+1) + '), '
            query_cypher = query_cypher[:-2]
            query_cypher += ' WHERE '
            for i in range(joins):
                query_cypher += '(apoc.text.levenshteinSimilarity(n' + str(i+1) + '.value, \"' + str(els_split[i][2]) + '\")) >= ' + str(sims[i]) + ' AND '
            ...
            for i in range(joins):
                if i in unbound_pos:
                    query += str(rels[i]) + '-*-1.0' + ','
                else:
                    query += str(rels[i]) + '-' + x_ids[i] + '-' + str(sims[i]) + ','
...
if __name__ == '__main__':
    ...
    df = pd.read_csv('Data/british_nodes_human.csv', header=None, sep='!__!')
    ...
    filename = 'training_wiki_star_super_sim_' + str(joins) + '.csv'

...
def run_process(i):
    ...
    while True:
        ...
        with open(filename, 'a') as data_file:
            ...
            query_cypher += ' WHERE '
            for i in range(joins):
                query_cypher += '(apoc.text.levenshteinSimilarity(x' + str(i+2) + '.rdfs__label, \"' + str(els_split[i][2]) + '\")) >= ' + str(sims[i]) + ' AND '
...
if __name__ == '__main__':
    ...
    df = pd.read_csv('Data/nodes_with_labels.csv', header=None, sep='!__!')
    ...
    filename = 'training_swdf_star_super_sim_' + str(joins) + '.csv'
